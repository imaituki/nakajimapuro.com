...
class AD_request {
	...
	// 主テーブル
	var $_CtrTable    = "t_request";
	var $_CtrTablePk  = "id_request";
	var $_CtrTable2   = "t_request_lecturer";
	var $_CtrTablePk2 = "id_request_lecturer";
	var $_CtrTable3   = "t_request_ojt";
	var $_CtrTablePk3 = "id_request_ojt";
	...
	//	$objInputCheck->entryData( "実施希望日時（時間）", "hope_time", $arrVal["hope_time"], array( "CHECK_EMPTY" ), null, null );
		...
		if( ( strcmp( $mode, "update" ) == 0 ) ) {
			$objInputCheck->entryData( "申込みID", "all", $arrVal["id_request"], array( "CHECK_EMPTY", "CHECK_NUM" ), null, null );
		}
		...
		// 取得
		if( ( strcmp( $mode, "update" ) == 0 ) ){
			$chk = $this->_DBconn->_ADODB->GetOne( "select id_request from t_request where class = " . $arrVal["class"] . " AND number = '" . $arrVal["number"] . "' AND request_year = " . $arrVal["request_year"] . " AND id_request != " . $arrVal["id_request"] );
		}else{
			$chk = $this->_DBconn->_ADODB->GetOne( "select id_request from t_request where class = " . $arrVal["class"] . " AND number = '".$arrVal["number"] ."' AND request_year = " . $arrVal["request_year"] . " " );
		}
	...
	function update( $arrVal, $arrSql = null ) {
		...
		// 更新条件
		$where = $this->_CtrTablePk . " = " . $arrVal["id_request"];
	...
	function updatelecturer( $arrVal, $arrSql = null ) {
		...
		// 更新条件
		$where = $this->_CtrTablePk2 . " = " . $arrVal["id_request_lecturer"];
	...
	function updateojt( $arrVal, $arrSql = null ) {
		...
		// 更新条件
		$where = $this->_CtrTablePk3 . " = " . $arrVal["id_request_ojt"];
	...
	function GetSearchList( $search, $option = null, $where = null, $select = null ) {
		...
		$creation_kit = array(  "select" => $this->_CtrTable . ".*, 
								...
								"join"   => "left join " . $this->_CtrTable2 . " on " . 
											$this->_CtrTable2 . ".id_request = " . $this->_CtrTable . ".id_request",
								"group"  => $this->_CtrTable . ".id_request ", 
		...
		// 年度
		if( !empty( $search["search_request_year"] ) ) {
			$creation_kit["where"] .= "AND request_year = " . $search["search_request_year"] . " ";
		}
	...
	function GetSearchLecturerList( $search ) {
		...
		$creation_kit = array(  "select" => "*",
								...
								"where"  => "id_request = " . $search["id_request"],
								"order"  => "id_request_lecturer ASC"
							);
